<?php
/**
 * Charts 4 PHP
 *
 * @author Shani <support@chartphp.com> - http://www.chartphp.com
 * @version 2.0
 * @license: see license.txt included in package
 */

include_once("../../config.php");
include_once(CHARTPHP_LIB_PATH . "/inc/chartphp_dist.php");

$p = new chartphp();

// data array is populated from example data file
include("../../example_data.php");
$p->data = $bubble_chart_data;
$p->chart_type = "bubble";

// Common Options
$p->title = "Life Expectancy v/s GDP per Capita";
$p->xlabel = "Life Expectancy";
$p->ylabel = "GDP per Capita";
$p->series_label = array("Asia", "Africa", "Europe", "Americas", "Oceania");

// $p->xrange = array(70,75); // for defining the xrange in numbers- set the xtype to null
// $p->xtype = null;
// $p->yrange = array(1000, 10000); // for defining the yrange in numbers- set the ytype to null
// $p->ytype = null;

$out = $p->render('c1');
?>
<!DOCTYPE html>
<html>
	<head>
		<link rel="stylesheet" href="../../lib/js/chartphp.css">
		<script src="../../lib/js/jquery.min.js"></script>
		<script src="../../lib/js/chartphp.js"></script>
	</head>
	<body>
		<div>
			<?php echo $out; ?>
		</div>
	</body>
</html>
